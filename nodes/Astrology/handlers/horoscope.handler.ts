...
async function handleSignOperation(
	...
	const targetDate = executeFunctions.getNodeParameter('targetDate', itemIndex) as string;
	const tradition = executeFunctions.getNodeParameter('tradition', itemIndex) as string;
	...
	if (isTextOperation) {
		body.format = executeFunctions.getNodeParameter('textFormat', itemIndex) as string;
		body.emoji = executeFunctions.getNodeParameter('emoji', itemIndex) as boolean;
	}

	return await makeApiRequest(executeFunctions, 'POST', baseUrl, endpoint, apiKey, body);
}
...
async function handlePersonalOperation(
	...
	// Add text-specific options
	if (isTextOperation) {
		additionalFields.format = executeFunctions.getNodeParameter('textFormat', itemIndex) as string;
		additionalFields.emoji = executeFunctions.getNodeParameter('emoji', itemIndex) as boolean;
	}
 ...
 */
async function handleChineseBaziOperation(context: IHandlerContext): Promise<IDataObject> {
	const { executeFunctions, itemIndex, baseUrl, apiKey } = context;

	const birthData = buildBirthData(executeFunctions, itemIndex);
	const baziYear = executeFunctions.getNodeParameter('baziYear', itemIndex) as number;
	const language = executeFunctions.getNodeParameter('language', itemIndex) as string;
